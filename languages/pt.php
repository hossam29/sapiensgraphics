<?php
	$lang = array(
        // All Pages
        "btnView" => "View Work",
        "btnHome" => "Go Home",
        "thanks" => "Thank you for taking the time to view my work. I hope you liked it!",
        "feelFree" => "Feel free to browse my work.",
        
        // Index Page
        // Titles/Headings
		"titleIndex" => "Sapien's Graphics | Welcome!",
        "titleIndexLanding" => "Sapien's Graphics' Portfolio",
        "titleIndexGeneralArtwork" => "General Artwork",
        "titleIndexLogoDesign" => "Logo Design",
        "titleIndexPaleoArt" => "Paleo Art",
        "titleIndexPixelArt" => "Pixel Art",
        "titleIndexFaq" => "FAQ",
        "titleIndexGit" => "Get In Touch",
        //Captions/Other
        "captionLanding" => "Digital Graphics Work",
        "captionGeneralArtwork" => "Click the image above to see my portfolio of general artwork.",
        "captionLogoDesign" => "Click the image above to see my portfolio of logo design.",
        "captionPaleoArt" => "Click the image above to see my portfolio of paleo art.",
        "captionPixelArt" => "Click the image above to see my portfolio of pixel art.",
        "captionFaq" => "Click the image above to see frequently asked questions.",
        "captionEmailEnquiries" => "Email me with any enquiries you have here:",
        
        // General Artwork
        "titleGeneralArtWork" => "General Artwork Portfolio",
        
        // Logo Work Page
        "titleLogoDesign" => "Logo Design Portfolio",
        
        // Paleo Art Page
        "titlePaleoArt" => "Paleo Art Portfolio",
        
        // Pixal Art Page
        "titlePixelArt" => "Pixel Art Portfolio",
        
        // FAQ Page
        "titleFaq" => "Frequently Asked Questions",
        "titleFindAnswers" => "Find answers to some of the frequently asked questions.",
        
        // Germanic Languages
        // West Germanic
		"lang_en" => "English",
        "lang_de" => "Deutsch",
        // North Germanic
        "lang_sv" => "Svenska",
        
        // Romance Languages
        //Ibero-Romance
        "lang_es" => "Español",
        "lang_pt" => "Português",
        //Gallo-Romance
        "lang_fr" => "Français",
        // Italo-Dalmatian Romance
        "lang_it" => "Italiano",
        //Eastern Romance
        "lang_ro" => "Limba Română",
        
        // Salvic Lanuages
        // East Slavic
        "lang_ru" => "Pyсский",
        //South Slavic
        "lang_bg" => "Български",
		"lang_bs" => "Босански",
        
        //East Asian Lanuages
        "lang_ko" => "한국어",
        "lang_jp" => "日本語",
        "lang_cmd" => "官话",
	);
?>